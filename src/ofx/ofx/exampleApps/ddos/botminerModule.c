...
void* statsThread(void *arg){
	...
	struct FlowEntryNoHash* flowArray = calloc(20000, sizeof(struct FlowEntry));
	...
	while(1){
		...
		for (flow = flow_HT; flow != NULL; flow=flow->hh.next){
			...
			memcpy(&flowArray[idx], flow, (sizeof(struct FlowEntry)-sizeof(UT_hash_handle)));
			idx +=1;
		}
		printf("copied %i flow entries.\n",idx);		
		printf("%i flow entries added to asic. (out of %i attempts)\n",flowsAddedToAsic, entryAttempts);		
		sendMsgUp((unsigned char *)flowArray, (sizeof(struct FlowEntry)-sizeof(UT_hash_handle))*(idx),0x30);
		sleep(1);
	}
