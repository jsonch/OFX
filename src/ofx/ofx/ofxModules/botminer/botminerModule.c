...
#define MODULEID 0x30
#define STATSMESSAGE 0x66 // Statistics message for switch manager.
...
// Botminer startup: do nothing.
int module_startup(){
	last_time = 0;
	current_time = 0;
	dprint("runnning data path start up function for botminer module.\n");
...
int handle_message(struct ofxDpHeader * msgHeader, unsigned char* msg){
	dprint("botminer module got a message in the dataplane.\n");
...
void* statsThread(void *arg){
	...
	while(1){
		dprint("gathering statistics about flows.\n");
		struct FlowEntry* flow;
		int idx = 0;
		...
		dprint("copied %i flow entries.\n",idx);		
		dprint("%i flow entries added to asic. (out of %i attempts)\n",flowsAddedToAsic, entryAttempts);		
		sendMsgUp((unsigned char *)flowArray, sizeof(struct FlowEntryNoHash)*(idx),MODULEID, STATSMESSAGE);
		sleep(10);
	}
